...
pub fn greater_than_builtin(interp: &mut Interpreter) -> Result<(), RuntimeError> {
    let b = interp.pop()?;
    let a = interp.pop()?;
...
mod tests {
    ...
    fn test_greater_than_true() {
        ...
        greater_than_builtin(&mut interp).unwrap();
    ...
    #[test]
    fn test_greater_than_stack_underflow() {
        let mut interp = setup_interpreter();

        let result = greater_than_builtin(&mut interp);
    ...
    #[test]
    fn test_greater_than_type_error() {
        let mut interp = setup_interpreter();
        interp.push(Value::Number(5.0));
