...
pub fn vector_builtin(interp: &mut Interpreter) -> Result<(), RuntimeError> {
...
pub fn vector_length_builtin(interp: &mut Interpreter) -> Result<(), RuntimeError> {
...
pub fn vector_ref_builtin(interp: &mut Interpreter) -> Result<(), RuntimeError> {
...
pub fn vector_to_list_builtin(interp: &mut Interpreter) -> Result<(), RuntimeError> {
    let vector_value = interp.pop()?;
...
pub fn list_to_vector_builtin(interp: &mut Interpreter) -> Result<(), RuntimeError> {
...
mod tests {
    ...
    fn test_vector_length_builtin() {
        ...
        interp.push(Value::Number(2.0));
        vector_builtin(&mut interp).unwrap();
        vector_length_builtin(&mut interp).unwrap();
    ...
    fn test_vector_ref_builtin() {
        ...
        interp.push(Value::Number(1.0));
        vector_ref_builtin(&mut interp).unwrap();
    ...
    fn test_vector_to_list_builtin() {
        ...
        vector_to_list_builtin(&mut interp).unwrap();
    ...
    fn test_list_to_vector_builtin() {
        ...
        interp.push(list);
        list_to_vector_builtin(&mut interp).unwrap();
    ...
    #[test]
    fn test_list_to_vector_requires_proper_list() {
        let mut interp = setup_interpreter();

        interp.push(Value::Number(10.0));
        let result = list_to_vector_builtin(&mut interp);
        assert!(matches!(
            result,
            Err(RuntimeError::TypeError(msg)) if msg.contains("proper list")
        ));
    }
}
